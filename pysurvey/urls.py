...
urlpatterns = [
    ...
    url(r'^studenti/(?P<cap>.*)/(?P<email>.*)/(?P<anno>.*)/(?P<citta>.*)/(?P<zona>.*)/(?P<grado_studi>.*)/(?P<voto>.*)/(?P<esami>.*)/(?P<campo_studi>.*)/(?P<livello_pc>.*)/(?P<lingua>.*)/(?P<conoscenza_specifica>.*)/(?P<stato>.*)/(?P<note>.*)/(?P<mansione_attuale>.*)/(?P<livello_cariera_attuale>.*)/(?P<ruolo_attuale>.*)/(?P<area_operativa_attuale>.*)/(?P<tipo_contratto_attuale>.*)/(?P<lavoro_passato>.*)/(?P<numero_attivita_svolte>.*)/(?P<mesi_attivita_svolte>.*)/(?P<descrizione_esperienza_pregressa>.*)/(?P<mansione_pregressa>.*)/(?P<livello_cariera_pregressa>.*)/(?P<ruolo_pregressa>.*)/(?P<area_operativa_pregressa>.*)/(?P<tipo_contratto_pregressa>.*)/(?P<mansione_futura>.*)/(?P<livello_cariera_futura>.*)/(?P<ruolo_futura>.*)/(?P<area_operativa_futura>.*)/(?P<tipo_contratto_futura>.*)/(?P<benefit>.*)/(?P<stipendio>.*)/(?P<interesse>.*)/(?P<possibilita_trasferirsi>.*)/(?P<data>.*)/$', jobsurvey.views.Studenti.as_view(), name='studenti'),
